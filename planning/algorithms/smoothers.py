...
def _mod2pi(theta):
    """Modulus operation to make sure any angle goes into the range [0, 2*pi) which is useful when talking about steering"""
...
# "d" is the distance between the start and goal divided by the turning radius
...
# Slightly different since there is no straight line connecting the turns, more optimal for short distance manoeuvre where the start and end are too close to allow for an outer or inner tangent
def _dubins_RLR(a, b, d):
    ...
    # The three turning circles must be mutually tangential so the centres form an isosceles triangle. This is the cosine of the angle related to the middle turn
    cos_middle = (6.0 - d * d + 2 * cab + 2 * d * (sa - sb)) / 8.0

    # If the distance between the center of the first and third circle is greater than 4R where R is the distance from the center of one of the circles to the outer edge of the second circle.
    # If the distance is greater than 4R then the middle circle can't touch both to bridge the gap
    if abs(cos_middle) > 1.0:
        return None

    # This is the travel distance along the middle circle arc
...
class Smoother(ABC):
    ...
    def _round_corners(self, path, grid):
        ...
        for run in sorted(runs, key=lambda r: r[0], reverse=True):
            ...
            if curve is not None and validate_path(grid, curve):
                ...
                points[left_boundary : right_boundary + 1] = [
                    np.asarray(c, dtype=float) for c in curve
                ]
    ...
    def _bezier_curve(control_points, n_samples):
        ...
        for fraction in np.linspace(0.0, 1.0, n_samples):
            ...
            while len(points) > 1:
                ...
                points = [
                    (1.0 - fraction) * points[i] + fraction * points[i + 1]
    ...
    @classmethod
    def _dubins_path(
        cls, start, end, radius, step, position_tolerance=0.75, angle_tolerance=0.30
    ):
        """
        ...
        for _, types, seg_lengths in candidates:
            ...
            if (
                math.hypot(final_x - end_x, final_y - end_y) <= position_tolerance
                and _angle_gap(final_theta, end_theta) <= angle_tolerance
            ):
                curve = []
...
class NoSmoother(Smoother):
    ...
    def smooth(self, path, grid):
        # Overrides the base smooth entirely so no corner is ever looked for, which is what makes this the control the other three are measured against
        return path
...
class BezierSmoother(Smoother):
    ...
    A single Bezier curve is fitted to the whole window, using every point in it as a control point. The curve is pinned to the two ends of the window and pulled towards the points in between without passing through them, which is what rounds the corner off.

    It is cheap and it looks smooth, but every control point influences the whole curve, so a change at one end of the window moves the curve at the other. It also says nothing about turning radius, so a rounded corner can still be tighter than a non-holonomic vehicle could drive. That is the gap the Dubins strategy below closes.
    """
...
class BSplineSmoother(Smoother):
    """
    B-spline corner rounding.

    A clamped cubic B-spline over the window, which is piecewise rather than one polynomial across the whole thing. Each span is shaped by only the four control points nearest to it, so the curve follows the original path more closely than a Bezier does and a change at one end of the window leaves the other end alone.

    It joins with continuous curvature rather than only continuous direction, which is the smoother of the two for steering. Like the Bezier it still gives no bound on how tight the turn can get.
    """
...
class DubinsSmoother(Smoother):
    """
    Dubins corner rounding.

    Unlike the other two this guarantees the result is driveable: a Dubins path is built from circular arcs of a fixed radius joined by straight lines, so its curvature never exceeds one over that radius by construction. A vehicle that can turn that tightly can follow the curve.

    It also needs more than the other two to work with. A Bezier only needs the points, whereas this needs a full pose at each end, meaning a position and a heading, because the shortest curvature-bounded path between two points depends on which way the vehicle is facing at both. The headings are taken from the segments entering and leaving the window.

    The cost is that it can fail where the others merely look bad. It only drives forwards, so where a street is tighter than the turning circle no word reaches the end pose and the curve comes back as None, leaving the original sharp corner in place.
    """

    def _curve(self, window):
        # The direction the path arrives on and the direction it leaves on, taken from the first and last segments of the window
        start_dir = window[1] - window[0]
        end_dir = window[-1] - window[-2]

        # A segment of no length has no direction, so there is no pose to aim from or at
        if np.linalg.norm(start_dir) < 1e-9 or np.linalg.norm(end_dir) < 1e-9:
            return None

        # Each end as a full pose: where it is, and which way it faces
        start_pose = (
            window[0][0],
            window[0][1],
            math.atan2(start_dir[1], start_dir[0]),
        )
        end_pose = (window[-1][0], window[-1][1], math.atan2(end_dir[1], end_dir[0]))

        # The shortest word that actually lands on the end pose, sampled into points. It returns None where the street is tighter than the turning circle
